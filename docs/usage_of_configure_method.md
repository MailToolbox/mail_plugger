...
With the `configure` method, we can add configurations for the MailPlugger.
...
There are 4 types of sending methods:
...
There is a limitation here. For example, if we select `round_robin` as a sending method, and we define `delivery_system` in the `Mail::Message` object, then it cannot choose delivery systems equally because it chooses a delivery system every single time, but the defined `delivery_system` in the `Mail::Message` object overrides it.

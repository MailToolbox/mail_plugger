...
With this Class, it can extract data from the `Mail::Message` object and send a message based on the given configurations. We can add these options directly in the `new` method, or we can use the `MailPlugger.plug_in` method as well.
...
If we are using the `mail_plugger` gem in Ruby on Rails, we don't have to do anything with this class. Rails will load this class automatically if we add this config `config.action_mailer.delivery_method = :mail_plugger`, e.g. into the `config/application.rb` file. Basically, we should use `MailPlugger.plug_in` method to configure this delivery method.
